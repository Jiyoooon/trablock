            ...
            >
              <v-list dense nav class="py-0">
                ...
                <v-list-item
                  ...
                  :key="item.title"
                  link
                  :to="{name: 'groupdetail',query: { groupId: item.id }}"
                >
                  <v-list-item-icon>
                    ...
                    <v-list-item>
                      <v-list-item-content>
                        ...
                        <v-row>
                          <v-col cols="12" sm="7">
                            <v-text-field
                              label="모임에 대한 간단한 설명을 입력해 주세요."
                              v-model="groupExplanation"
                              class="ml-15"
                            ></v-text-field>
                          </v-col>
                        </v-row>
                            ...
                            <v-card>
                              ...
                              <v-card-actions>
                                <v-btn
                                  color="blue darken-1"
                                  text
                                  @click="dialogUser = false; pickedFriend = []"
                                >Close</v-btn>
                        ...
                        <v-row>
                          <v-col cols="12" sm="3">
                            <div class="subheading ml-14">
                              <small>정기 납부를 종료할 날짜를 선택해 주세요.</small>
                            </div>
                            <v-date-picker
                              v-model="finished"
                              color="amber darken-1"
                              class="ml-15"
                              @change="updategroupDates()"
                            ></v-date-picker>
                          </v-col>
                          <v-col cols="12" sm="9">
                            <v-row>
                            ...
                            <v-row>
                              ...
                              <v-col cols="12" sm="2">
                                <v-select :items="cycle" label="납입 주기" v-model="groupCycle"></v-select>
                              </v-col>
                          ...
                          <v-col cols="12" sm="9" class="mt-15">
                            <v-row>
                              <v-col cols="12" sm="4">
                                <v-text-field
                                  label="여행지를 입력해 주세요."
                                  v-model="groupDestinationCountry"
                                  class="ml-15"
                                  hint="국가명"
                                ></v-text-field>
                              </v-col>
                        ...
                        <v-row>
                          <v-col align="right">
                            <router-link :to="{name: 'groupmain'}" class="text-decoration-none">
                              <v-btn color="amber darken-3" text @click="dialogUser = false">
                                <h3>Close</h3>
                              </v-btn>
                            </router-link>
                            <v-btn color="amber darken-3" text @click="dialogCreate = true">
                              <h3>Save</h3>
                            </v-btn>
                          </v-col>
                          <v-dialog v-model="dialogCreate" max-width="500px">
                            <v-card>
                              <v-card-title>모임 계좌를 생성하는 과정입니다.</v-card-title>
                              <v-card-text>
                                <v-text-field label="개인키를 입력하세요." v-model="groupKey"></v-text-field>
                              </v-card-text>
                              <v-card-actions>
                                <v-btn color="green" text @click="createGroup()">create</v-btn>
  ...
  methods: {
    searchUser() {
      ...
      this.userList.forEach(element => {
        if (
          element.email.includes(this.friend) ||
          element.nickname.includes(this.friend)
        ) {
          this.sUserList.push(element);
        }
      });
    ...
    createGroup() {
      if (this.groupType.length == "") {
        ...
        alert(
          "퇴출 수수료를 입력해주세요. 퇴출 수수료는 무단으로 모임을 나가거나 장기적으로 정기납부를 하지 않은 인원에게 부가되는 수수료입니다. 이러한 인원이 퇴출 시 현재까지 낸 금액의 퇴출수수료(%)만큼 제하고 퇴출 처리 됩니다."
        );
      } else {
        var canMake = false;
          ...
          if (ok) {
            ...
            http
              .post("/party", {
                ...
                totalAmount: 0,
                payCycle: this.groupCycle == "월간" ? false : true,
                ...
                destination:
                  this.groupDestinationCountry +
                  " " +
                  this.groupDestinationCity,
                available: true,
    ...
    updateRegularPay() {
      if (this.groupCycle == "월간") {
        ...
        if (cnt == 0) this.groupRegularPay = "모금 기간을 늘려주세요.";
          ...
          this.groupRegularPay = Math.round(
            this.groupTarget / cnt / (this.pickedFriend.length + 1)
          );
        ...
        if (this.groupPayDate == "일요일") {
          this.pickedYoilIndex = 0;
        } else if (this.groupPayDate == "월요일") {
