                        ...
                        >
                          ...
                          <h4 class="mx-8 my-3">
                            <span v-if="realBadMember.length > 0">
                              <v-icon color="red" class="mr-2"
                                >mdi-cancel</v-icon
                              >
                              <span v-for="item in realBadMember" :key="item"
                                >{{ item }},</span
                              >님은 미납한 경험이 있는 회원입니다.
                            </span>
                          </h4>
                          <h4 class="mx-8 my-3">
                            <span v-if="goodMember.length > 0">
                              ...
                              >
                              <span v-for="item in goodMember" :key="item"
                                >{{ item }}, </span
                              >님이 정상납부하였습니다.
                            </span>
                          </h4>
                          <h4 class="mx-8 my-3">
                            <span v-if="badMember.length > 0">
                              ...
                              >
                              <span v-for="item in badMember" :key="item"
                                >{{ item }},</span
                              >님이 아직 미납하였습니다.
                            </span>
                          </h4>
...
export default {
  ...
  created() {
    ...
    http
      ...
      .then(({ data }) => {
        ...
        if (data.withdraw) {
          var ok = confirm(
            this.rich + "이 " + this.richAmount + "의 출금을 요청합니다."
          );
          if (ok) {
  ...
  methods: {
    ...
    calculateNextPayDate() {
      ...
      // 3. 오늘 날짜
      var endDate = this.groups.endDate; // yyyy-mm-dd
      var today = this.getTimeStamp(); // yyyy-mm-dd
      var cycleIsWeek = this.groups.payCycle; // False : 월단위, True : 주단위
      var cycle = this.groups.payDate; // F : 1~28, 주 : 0(일) ~ 6(토)
      var result;
      ...
      if (cycleIsWeek) {
        ...
        if (dateToday > cycle) {
          var _split = today.split("-"); // 년,월,일로 나눠서 계산
          if (_split[1] == 12) {
            // 12월인 경우 1달 후는 내년 1월이다
            _split[0] += 1;
            _split[1] = 0;
          }
          result = _split[0] + "-" + (_split[1] + 1) + "-" + cycle; // 오늘 날짜(년, <월 + 1달 후>) + 일(특정일자)
          ...
          result = _split[0] + "-" + _split[1] + "-" + cycle; // 오늘 날짜(년, 월) + 일(특정일자)
    ...
    isEnd(day, end) {
      ...
      var __day = new Date(_day[0], _day[1], _day[2]).getTime();
      var __end = new Date(_end[0], _end[1], _end[2]).getTime();
